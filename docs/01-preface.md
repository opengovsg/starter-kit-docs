...
For serious production use cases, consider directly deploying the Starter Kit application to AWS (and skipping the infrastructure part of these documentation). We provide [a command line interface (CLI) tool](./guides/migrate-vercel-to-aws.md#cli) (for internal use only) that can help you deploy the Starter Kit to AWS.

Nonetheless, on low-risk production use cases, the Starter Kit infrastructure (with Vercel) can still be used as a production deployment. We have a document on when one might want to migrate to AWS [here](./guides/migrate-vercel-to-aws.md).
